  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">🦠 Disease Intelligence</h1>
          <p className="text-dark-400 text-sm mt-1">Search and analyze disease surveillance data</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1">
          <h2 className="text-sm font-semibold text-dark-400 uppercase tracking-wider mb-3">Tracked Diseases</h2>
          <div className="space-y-1 max-h-[600px] overflow-y-auto pr-1">
            {loading && diseases.length === 0 ? (
              ...
              <p className="text-dark-400 text-sm py-4 text-center">No diseases found</p>
            ) : (
              diseases.map((disease) => (
                ...
                >
                  ...
                  {disease.category && (
                    <span className="block text-[11px] text-dark-500 mt-0.5">{disease.category}</span>
                  )}
                </button>
              ))
            )}
          ...
          {selectedDisease ? (
            <>
              <div className="glass-card p-5">
                <h2 className="text-xl font-bold text-white mb-1">{selectedDisease}</h2>
                <p className="text-dark-400 text-sm">
                  {diseases.find(d => d.name === selectedDisease)?.description || 'Disease surveillance data'}
                </p>
              </div>
              ...
              {predictions.length > 0 && (
                <div className="glass-card p-5">
                  <h3 className="text-sm font-semibold text-white mb-4">📈 7-Day Case Prediction</h3>
                  <TrendChart
                    labels={predictions.map((p: any) => new Date(p.prediction_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }))}
                    datasets={[
                      {
                        label: 'Predicted Cases',
                        data: predictions.map((p: any) => p.predicted_cases || 0),
                        color: '#6366f1',
                      },
                      {
                        label: 'Upper Bound',
                        data: predictions.map((p: any) => p.confidence_upper || 0),
                        color: '#ef4444',
                        fill: false,
                      },
                      {
                        label: 'Lower Bound',
                        data: predictions.map((p: any) => p.confidence_lower || 0),
                        color: '#10b981',
                        fill: false,
                      },
                    ]}
                  ...
                  <div className="flex items-center gap-4 mt-3 text-[11px] text-dark-500">
                    <span>Model: {predictions[0]?.model_type?.toUpperCase() || 'LSTM'}</span>
            ...
            <div className="glass-card p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-white mb-2">Select a Disease</h3>
              <p className="text-dark-400 text-sm">Choose a disease from the list or search to view detailed surveillance data.</p>
            </div>
          )}
        </div>
      </div>
